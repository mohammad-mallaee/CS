...
def parse_account_lines(lines):
    ...
    for line in lines:
        ...
        accounts[int(account_number)] = float(account_balance)
    return accounts
...
def get_accounts_by_string():
    ...
    balances_str = input()
    return parse_accounts(accounts_str, balances_str)


def get_accounts():
    try:
        with open(ACCOUNTS_FILE_PATH, "r+") as accounts_file:
            account_lines = accounts_file.readlines()
...
def save_accounts(accounts):
    with open(ACCOUNTS_FILE_PATH, "w") as f:
        for account_number in accounts:
            f.write(str(account_number) + " " + str(accounts[account_number]) + "\n")
